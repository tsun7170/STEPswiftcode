...
static void printOutHeaderComment(void) {
	char ** hp;
	error_sym.line = 1;
...
void openSwiftFileForSchema(Schema schema) {
	...
	char buf[BUFSIZ];
	snprintf( swift_filename_buffer, sizeof swift_filename_buffer, "%s.swift", SCHEMA_swiftName(schema, buf) );
...
static void openSwiftFile(Schema s, const char* category, char initial, const char* filename) {
	...
	snprintf( swift_filename_buffer, sizeof swift_filename_buffer, "%s/%s.swift", dir2, filename );
	error_sym.filename = swift_filename_buffer;
